...
func (f FixturePostgreSQL) Name() string {
	return "postgresql"
}

func (f *FixturePostgreSQL) resolve(filename string) error {
...
// check the postgresql fixture configs
func (f FixturePostgreSQL) check(filename string) error {
...
func (f FixturePostgreSQL) checkSource(filename string) error {

	sources, ok := f.Source.([]interface{})
	if !ok {
		return fmt.Errorf("invalid PostgreSQL fixture source value on `%s`", filename)
	}
	...
	for index, source := range sources {
		...
		if !ok {
			return fmt.Errorf("invalid PostgreSQL fixture source value on `%s` index %d", filename, index)
		}
		...
		if val, ok := castedSource[definition]; ok {
			if len(castedSource) > 2 {
				return fmt.Errorf("ambiguous PostgreSQL source name on `%s` index %d", filename, index)
			} else if _, ok := val.(string); !ok {
				return fmt.Errorf("source definition for PostgreSQL is not a valid string on `%s` index %d", filename, index)
			}
		...
		for key, value := range castedSource {
			...
			if _, ok := key.(string); !ok {
				return fmt.Errorf("invalid PostgreSQL fixture source key on `%s` index %d", filename, index)
			}
			if value == nil {
...
func (f FixturePostgreSQL) checkDestination(filename string) error {
	if err := basicStringTypeCheck(f.Destination); err != nil {
		return fmt.Errorf("%s PostgreSQL fixture destination: %s", filename, err)
	}
	return nil
}

// parse the config into usable data types
func (f *FixturePostgreSQL) parse(filename string) {
...
func (f *FixturePostgreSQL) parseSource(filename string) {
...
func (f *FixturePostgreSQL) parseDestination() {
